...
class TestMCPToolHandlers:
	...
	def test_list_projects_empty(self, registry):
		from autodev.mcp_server import _dispatch
	...
	@patch("autodev.mcp_server.subprocess", create=True)
	def test_launch_mission(self, mock_sub, registry, sample_config):
		registry.register(name="test", config_path=str(sample_config))

		from autodev.mcp_server import _dispatch
		with patch("autodev.launcher.subprocess.Popen") as mock_popen:
			mock_proc = MagicMock()
			mock_proc.pid = 12345
	...
	def test_web_research_dispatch(self, registry):
		from autodev.mcp_server import _dispatch
		with patch("autodev.mcp_server._tool_web_research") as mock_wr:
...
class TestWebResearch:
	...
	def test_general_search_with_abstract(self):
		from autodev.mcp_server import _tool_web_research
		ddg_response = {
			"Heading": "Python",
		...
		with patch("autodev.mcp_server.urllib.request.urlopen", return_value=_mock_urlopen(ddg_response)):
	...
	def test_pypi_search(self):
		...
		with patch("autodev.mcp_server.urllib.request.urlopen", return_value=_mock_urlopen(pypi_response)):
	...
	def test_github_search(self):
		...
		with patch("autodev.mcp_server.urllib.request.urlopen", return_value=_mock_urlopen(gh_response)):
	...
	def test_url_fetch_html(self):
		...
		with patch("autodev.mcp_server.urllib.request.urlopen", return_value=_mock_urlopen(html, "text/html")):
	...
	def test_url_fetch_json(self):
		from autodev.mcp_server import _tool_web_research
		data = {"key": "value"}
		with patch("autodev.mcp_server.urllib.request.urlopen", return_value=_mock_urlopen(data)):
	...
	def test_url_fetch_truncation(self):
		...
		with patch("autodev.mcp_server.urllib.request.urlopen", return_value=mock_resp):
	...
	def test_network_error(self):
		...
		from autodev.mcp_server import _tool_web_research
		with patch("autodev.mcp_server.urllib.request.urlopen", side_effect=urllib.error.URLError("timeout")):
			result = _tool_web_research({"query": "fail"})
		assert "error" in result
		assert "Network error" in result["error"]

	def test_tool_in_tools_list(self):
		from autodev.mcp_server import TOOLS
