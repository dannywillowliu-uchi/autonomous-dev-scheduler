...
def _find_conn_execute_calls(filepath: Path) -> list[int]:
	...
	for node in ast.walk(tree):
		...
		if not isinstance(conn, ast.Attribute) or conn.attr != "conn":
			continue
		db_node = conn.value
		if not isinstance(db_node, ast.Attribute) or db_node.attr != "db":
			continue
		self_node = db_node.value
...
def test_no_direct_conn_execute_outside_db():
	...
	assert py_files, f"No .py files found under {_SRC_DIR}"

	all_violations: list[str] = []
	for filepath in py_files:
		if filepath.name == "db.py":
			continue
		if filepath.name in _LOCK_ALLOWLIST:
			continue
...
def test_continuous_controller_has_zero_conn_execute():
	...
	cc_path = _SRC_DIR / "continuous_controller.py"
