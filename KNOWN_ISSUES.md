...
**Symptom**: After autodev/green reset, `pip show autodev` points to `.mc-pool-ads/worker-XXXXX` instead of source tree. `Path(__file__)` resolves to the clone, breaking dashboard UI file lookup and any import that depends on package location.
...
**Symptom**: Every auto-push silently failed. `push_green_to_main()` fetch returned non-fast-forward error because autodev/green was reset at mission start but `refs/autodev/green-push` still pointed to previous mission's history.
**Fix**: Added `+` prefix to refspec: `+autodev/green:refs/autodev/green-push`.
...
### 5. Dashboard requires auth token in URL but `autodev live` output gets swallowed
**Status**: Unfixed
**Files**: `cli.py:cmd_live`
**Symptom**: When `autodev live` runs as a background process, the token URL printed to stdout is lost. User can't access the dashboard without the token.
**Fix needed**: Write token URL to a well-known file (e.g., `.mc-dashboard-url`) or a temp file, in addition to printing it.

### 6. Dashboard `autodev live` uses relative config path
**Status**: Unfixed
...
**Fix needed**: Resolve config path to absolute before deriving DB path, or search upward for `autodev.toml`.

## Low (cosmetic / noisy logs)

### 7. Non-fast-forward fetch errors in logs
**Status**: Will be fixed by #3's fix propagating
**Symptom**: Every merge logs `ERROR: Failed to fetch autodev/green: [rejected] non-fast-forward`. Noisy but non-blocking since the merge itself succeeds.
