...
from autodev.config import MissionConfig, claude_subprocess_env
...
class LocalBackend(WorkerBackend):
	...
	async def provision_workspace(
		...
		# autodev/green from GreenBranchManager.initialize()) are visible.
		...
		# of the source tree, breaking imports after autodev/green reset.
	...
	async def _repair_editable_install(self, source_repo: str) -> None:
		...
		source_venv = Path(source_repo) / ".venv"
		pth_files = list(source_venv.glob("lib/*/site-packages/__editable__.autodev*.pth"))
		if not pth_files:
			return
		pth = pth_files[0]
		try:
