...
def _parse_planner_output(output: str) -> PlannerResult:
	...
	# 2. Try PLAN_RESULT: marker (matches session.py AD_RESULT pattern)
...
acceptance_criteria MUST use project-relative paths only. NEVER /tmp/ or other absolute paths.
...
- NEVER use absolute paths (e.g. /tmp/foo.c). Use project-relative paths only.
- Use pytest or the project's test runner, not ad-hoc file creation outside the project.
- Example good: ".venv/bin/python -m pytest tests/test_foo.py -q"
- Example bad: "echo 'int main(){{}}' > /tmp/test.c && python -m compiler /tmp/test.c -o /tmp/test.s"
