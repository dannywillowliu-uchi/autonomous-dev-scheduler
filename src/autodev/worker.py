...
- Include research findings and MCP tool usage in the 'discoveries' field of AD_RESULT

## Output
When done, write a summary as the LAST line of output:
AD_RESULT:{{"status":"completed|failed|blocked","commits":["hash"],\
...
def _build_handoff(mc_result: dict[str, object], work_unit_id: str, round_id: str) -> Handoff:
	"""Build a Handoff from parsed AD_RESULT data.
...
- Document findings thoroughly in your AD_RESULT discoveries field
...
## Output
When done, write a summary as the LAST line of output:
AD_RESULT:{{"status":"completed|failed|blocked","commits":[],\
...
- After testing all approaches, produce a JSON comparison report in your AD_RESULT
...
- Document all findings thoroughly in your AD_RESULT discoveries field
...
- Document decisions with clear rationale in your AD_RESULT discoveries field
...
AD_RESULT:{{"status":"completed","commits":[],"summary":"architectural analysis",\
...
def render_mission_worker_prompt(
	...
	if project_root is not None:
		try:
			from autodev.snapshot import build_incremental_snapshot
...
def parse_handoff(output: str, work_unit_id: str, round_id: str) -> Handoff | None:
	"""Parse AD_RESULT from output and build a Handoff model.

	Returns None if no AD_RESULT found in the output.
	"""
