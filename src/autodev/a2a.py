"""Agent-to-Agent (A2A) protocol v0.3 support for autodev.
...
if TYPE_CHECKING:
	from autodev.config import A2AConfig
...
class AgentCard(BaseModel):
	...
	name: str
	version: str
	description: str = "autodev autonomous development agent"
	...
	def _build_app(self) -> Any:
		try:
			...
			raise ImportError(
				...
				"Install with: pip install autodev[dashboard]"
			)

		app = FastAPI(title="autodev A2A", version="0.3")
		config = self._config
		db = self._db
		...
		def create_task(req: A2ATaskCreate) -> dict[str, Any]:
			from autodev.models import Plan, WorkUnit, _now_iso
