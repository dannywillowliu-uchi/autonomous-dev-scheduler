...
from autodev.backends import ContainerBackend, LocalBackend, SSHBackend, WorkerBackend
from autodev.batch_analyzer import BatchAnalyzer
from autodev.causal import CausalAttributor, CausalSignal
from autodev.circuit_breaker import CircuitBreakerManager
from autodev.config import ContinuousConfig, MissionConfig, build_claude_cmd, claude_subprocess_env
...
from autodev.core_tests import (
	format_for_feedback_context,
	format_for_planner,
	run_core_tests,
	store_core_test_experience,
)
from autodev.criteria_validator import Severity, validate_criteria
from autodev.db import Database
from autodev.degradation import DegradationManager, DegradationTransition
from autodev.deliberative_planner import DeliberativePlanner
from autodev.diff_reviewer import DiffReviewer
...
from autodev.overlap import _parse_files_hint, topological_layers
...
from autodev.token_parser import compute_token_cost, parse_stream_json
from autodev.trace_log import TraceEvent, TraceLogger
from autodev.tracing import MissionTracer
...
def _update_core_test_baseline(project_path: str | Path, baseline_path: str) -> None:
	"""Copy results.json to baseline.json to update the high-water mark."""
	import shutil
	baseline = Path(project_path) / baseline_path
	results = baseline.parent / "results.json"
	if results.exists() and baseline_path:
		shutil.copy2(results, baseline)
		logger.info("Updated core test baseline from results")
...
class ContinuousController:
	...
	def __init__(self, config: MissionConfig, db: Database, chain_id: str = "") -> None:
		...
		try:
			if config.episodic_memory.enabled is True:
				from autodev.memory import MemoryManager
	...
	async def run(self, dry_run: bool = False) -> ContinuousMissionResult:
		...
		try:
			...
			# Run final verification on autodev/green
	...
	async def _init_components(self) -> None:
		...
		if hitl.push_gate.enabled or hitl.large_merge_gate.enabled:
			from autodev.hitl import ApprovalGate
	...
	async def _orchestration_loop(
		...
		while self.running:
			...
			state = build_planner_context(self.db, mission.id, core_test_context=self._core_test_feedback)
			...
			core_test_feedback_str = ""
			ct_cfg = self.config.core_tests
			if ct_cfg.enabled and ct_cfg.runner_command:
				try:
					ct_results = await run_core_tests(
						str(self.config.target.resolved_path),
						runner_command=ct_cfg.runner_command,
						baseline_path=ct_cfg.baseline_path,
						timeout=ct_cfg.timeout,
					)
					if ct_results is not None:
						core_test_results_str = format_for_planner(ct_results)
						core_test_feedback_str = format_for_feedback_context(ct_results)
						completed_units = [u for u in units if u.id in self._completed_unit_ids]
						store_core_test_experience(self.db, ct_results, epoch.id, completed_units)
						if ct_results.delta_passed > 0 and ct_results.delta_failed == 0 and ct_cfg.baseline_path:
							_update_core_test_baseline(self.config.target.resolved_path, ct_cfg.baseline_path)
				except Exception as exc:
					logger.warning("Core test feedback failed: %s", exc)
				self._core_test_feedback = core_test_feedback_str
			...
			try:
				update_mission_state(
					...
					core_test_results=core_test_results_str,
				)
			except Exception as exc:
				logger.warning("Failed to update MISSION_STATE.md: %s", exc)
...
OBJECTIVE_CHECK:{{"met": false, "reason": "what still needs to be done"}}"""
	...
	async def _blocking_review(
		...
		try:
			...
			try:
				from autodev.snapshot import get_project_snapshot
	...
	async def _review_merged_unit(
		...
		try:
			# Get the diff from the merge commit (not autodev/green HEAD which may be MISSION_STATE.md)
			if merge_commit_hash:
				diff_cmd = ["git", "diff", f"{merge_commit_hash}^..{merge_commit_hash}", "--", "."]
			else:
				green_branch = self.config.green_branch.green_branch
	...
	async def _resume_worker_for_fixup(
		...
		for attempt in range(max_fixup_attempts):
			...
			# Sync latest autodev/green into the worker's workspace so it can
	...
	def _schedule_retry(
		...
		"""
		from autodev.feedback import diagnose_failure
	...
	async def _retry_unit(
		...
		try:
			...
			from autodev.memory import load_context_for_mission_worker
	...
	def _extract_knowledge(self, unit: WorkUnit, handoff: Handoff | None, mission: Mission) -> None:
		...
		if not content:
			return
		from autodev.models import KnowledgeItem
		item = KnowledgeItem(
			mission_id=mission.id,
	...
	async def _execute_single_unit(
		...
		try:
			...
			knowledge_items = self.db.get_knowledge_for_mission(mission.id)
			if knowledge_items:
				from autodev.overlap import _parse_files_hint as _parse_scope
	...
	async def _run_final_verification(self) -> tuple[bool, str]:
		"""Run full verification on autodev/green at mission end.
		...
		# Checkout autodev/green in the green branch workspace
