"""SQLite database operations for autodev state."""
...
from autodev.constants import EVENT_TO_STATUS
...
class Database:
	"""SQLite database for autodev state."""
	...
	def _migrate_acceptance_criteria_columns(self) -> None:
		"""Add acceptance_criteria to work_units and criteria_met_score to unit_reviews (idempotent)."""
		migrations = [
	...
	def claim_work_unit(self, worker_id: str, now: str | None = None) -> WorkUnit | None:
		...
		"""
		if now is None:
			from autodev.models import _now_iso
			now = _now_iso()
		...
		if unit.epoch_id:
			...
			from autodev.constants import UNIT_EVENT_CLAIMED
			self.conn.execute(
