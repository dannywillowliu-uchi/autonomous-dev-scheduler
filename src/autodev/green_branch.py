"""Green Branch Pattern for autodev.

Manages autodev/green branch. Workers produce unit branches that merge directly
into autodev/green. Verification runs once at mission end.
"""
...
from autodev.models import VerificationNodeKind, VerificationReport, WorkUnit
from autodev.state import run_verification_nodes
...
class UnitMergeResult:
	"""Result of merging a single work unit into autodev/green."""
...
class GreenBranchManager:
	"""Manages the autodev/green branch lifecycle."""
	...
	async def initialize(self, workspace: str) -> None:
		"""Create autodev/working and autodev/green branches if they don't exist.
	...
	async def merge_unit(
		...
		"""Merge a unit branch into autodev/green.
	...
	async def merge_batch(
		...
		Flow:
		  1. Create speculative branch from autodev/green
		  2. Rebase + merge ALL units onto speculative branch (under lock)
		  3. Run verification ONCE on the combined result (outside lock)
		  4. If passes: fast-forward autodev/green to speculative branch, return all success
		...
		async with self._merge_lock:
			# Create speculative branch from current autodev/green
			...
			for i, (ws, branch, _ac) in enumerate(units):
				...
				rebase_branch = f"autodev/rebase-{branch}"
		...
		# Phase 3: fast-forward autodev/green to speculative branch (under lock)
		async with self._merge_lock:
			await self._run_git("checkout", gb.green_branch)
			...
			if not ok:
				logger.error("Failed to merge speculative batch into autodev/green: %s", output)
				await self._run_git("branch", "-D", spec_branch)
	...
	async def _merge_git_ops(
		...
		async with self._merge_lock:
			...
			try:
				...
				rebase_branch = f"autodev/rebase-{branch_name}"
				await self._run_git("branch", "-D", rebase_branch)
		...
		# Lock released -- merge commit is on autodev/green
		return None
	...
	async def run_fixup(self, failure_output: str) -> FixupResult:
		...
		passing, fewest lint errors, smallest diff on tie) is merged into autodev/green.
		"""
		gb = self.config.green_branch
		...
		# Merge the winner branch into autodev/green
		async with self._merge_lock:
	...
	async def _run_fixup_candidate(
		...
		# Create candidate branch from autodev/green
	...
	async def _sync_to_source(self) -> bool:
		"""Sync autodev/green and autodev/working refs from workspace clone to source repo.
		...
		for branch in (gb.green_branch, gb.working_branch):
			# Force-update (+) because workspace autodev/green always has the latest merges
	...
	async def push_green_to_main(self) -> bool:
		"""Merge autodev/green into the push branch and push to origin.
		...
		green_ref = "refs/autodev/green-push"

		# Fetch autodev/green into a named ref (FETCH_HEAD gets overwritten by pull)
		# Force-update (+) because autodev/green is reset at each mission start
		ok, output = await self._run_git_in(
			source_repo, "fetch", self.workspace,
			f"+{gb.green_branch}:{green_ref}",
		)
		if not ok:
			logger.error("Failed to fetch autodev/green: %s", output)
			return False
		...
		try:
			...
			if not ok:
				...
				if not ok:
					logger.error("Failed to merge autodev/green into %s: %s", push_branch, output)
					return False
			...
			logger.info("Pushed autodev/green to origin/%s", push_branch)
			return True
		finally:
			if stashed:
	...
	async def get_green_hash(self) -> str:
		"""Return the current commit hash of autodev/green."""
		gb = self.config.green_branch
	...
	def _workspace_env(self) -> dict[str, str]:
		...
		PYTHONPATH ensures imports resolve to the merged code on autodev/green rather
	...
	async def _run_verification(self) -> VerificationReport:
		...
		"""
		from autodev.state import _build_result_from_single_command
